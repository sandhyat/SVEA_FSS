...
for i in range(iterations):
    ...
    # TRAINING DATA
    training_data_file =  path_to_data + FILENAME + "_train.txt"
    pd.DataFrame(datasetFull).to_csv(training_data_file, index=False,header=False)
    # pd.DataFrame(dataset_train).to_csv(training_data_file, index=False, header=False)
...
plt.plot(feature_set,mean_accu_svea_exact,colors[2] + shape[2])
...
plt.savefig(path_to_data+FILENAME+"comparison_fig.png", bbox_inches= 'tight')
plt.close()
